...
import { X, Bot, TreePine } from "lucide-react"
import { Button } from "@/components/ui/button"

interface EatoBoticsModalProps {
  isOpen: boolean
  onClose: () => void
}

export default function EatoBoticsModal({ isOpen, onClose }: EatoBoticsModalProps) {
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div
                ...
                >
                  ...
                  <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-bold mb-4 sm:mb-6 leading-relaxed tracking-tight">
                    <div className="flex items-center justify-center flex-wrap gap-2 sm:gap-3 mb-2 px-4">
                      <Bot className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 text-orange-300 flex-shrink-0 drop-shadow-lg" />
                      <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-200 via-red-200 to-pink-300 whitespace-nowrap py-2 leading-normal drop-shadow-2xl">
                        EatoBotics
                      </span>
                    </div>
                  ...
                  <div className="bg-white/20 backdrop-blur-sm rounded-2xl p-6 mb-8 border border-white/30 max-w-4xl mx-auto">
                    <p className="text-lg text-gray-50 font-medium mb-4 drop-shadow-lg">
                      EatoBotics is currently in development as part of the EatoSystem
                    </p>
                    <p className="text-base text-gray-100 drop-shadow-lg">
                      This comprehensive robotic automation platform is being developed from zero to create the
                      definitive agricultural robotics ecosystem. Click continue to explore the main EatoSystem site
                      while EatoBotics is being finalized.
                    </p>
                  </div>
